...
Write-Host "[1/3] 下载主脚本 -> $dir\po0fw.ps1"
$local = Join-Path $PSScriptRoot "po0fw.ps1"
if (Test-Path $local) {
    Copy-Item $local (Join-Path $dir "po0fw.ps1") -Force
} else {
    Invoke-WebRequest -Uri "$RawBase/windows/po0fw.ps1" -OutFile (Join-Path $dir "po0fw.ps1")
}

Write-Host "[2/3] 写配置"
Set-Content -Path (Join-Path $dir "po0fw.conf") -Value $Tokens -Encoding UTF8

Write-Host "[3/3] 注册计划任务（每 10 分钟 + 网络切换）"
$action = New-ScheduledTaskAction -Execute "powershell.exe" `
    -Argument "-NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File `"$dir\po0fw.ps1`""
